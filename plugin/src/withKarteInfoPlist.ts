...
export const withKarteInfoPlist: ConfigPlugin<ConfigProps> = (config, props) => {
    return withXcodeProject(config, config => {
        ...
        config.modResults = setKarteInfoPlist(config.modRequest.projectRoot, config.modResults, props.karteInfoPlist);
        return config;
    });
};

function setKarteInfoPlist(projectRoot: string, project: XcodeProject, karteInfoPlistPath: string): XcodeProject {
